...
type dnsOverHTTPS struct {
	...
	// userAgent is a custom User-Agent string to use in HTTP headers.
	userAgent string
}
...
func (p *dnsOverHTTPS) exchangeHTTPSClient(
	...
	// Set User-Agent header: use custom value if provided, otherwise
	// prevent the client from sending User-Agent header, see
	// https://github.com/AdguardTeam/dnsproxy/issues/211.
	if p.userAgent != "" {
		httpReq.Header.Set(httphdr.UserAgent, p.userAgent)
	} else {
